...
export const FilteredProductList = ({ shopId }: FilteredProductListProps) => {
  ...
  const params = {
    shop_ids: shopId ? [shopId] : searchParams.getAll("shop_id"),
  ...
  let tempProductVariant = productVariant;
  if (productVariant === "1" && settings?.ui_type === "3") {
    tempProductVariant = "5";
  }

  if (productVariant === "1" && settings?.ui_type === "4") {
    tempProductVariant = "6";
  }
  if (isLoading) {
    return (
    ...
    <InfiniteLoader loadMore={fetchNextPage} hasMore={hasNextPage} loading={isFetchingNextPage}>
      <div className={clsx("grid", listStyles[productVariant as keyof typeof listStyles])}>
        {productList?.map((product) => (
          <ProductCard roundedColors data={product} variant={tempProductVariant} key={product.id} />
        ))}
      </div>
    </InfiniteLoader>
  );
