...
import { Types } from "@/app/(store)/(navigation)/cart/components/checkout/checkout.reducer";
...
export const CheckoutDeliveryForm = ({ onNext, isPageChanging }: CheckoutScreenProps) => {
  ...
  if (addressList?.length === 0 && !notAvailable) {
    return <AddressCreate deliveryPrice={deliveryPrices?.data?.[0]} onSuccess={() => onNext()} />;
  }
  ...
  return (
    <div className="mt-7">
          ...
          >
            {addressList?.map((address) => (
              <RadioGroup.Option key={address.id} value={address} className="cursor-pointer">
                {({ checked }) => (
                  <div className="flex items-center justify-between border-b border-gray-layout py-3">
      ...
      <Button
        disabled={!state.deliveryAddress || notAvailable}
